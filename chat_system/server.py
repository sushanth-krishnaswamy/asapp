...
class Server_Chat(object):
    ...
    def __init__(self, address='127.0.0.1', port=3490):
        ...
        # This is the output socket list
        self.outputs = []
        ...
        print 'Listening to port on port: ', port, '...'
    ...
    def sighandler(self, signum, frame):
        ...
        # Close the existing client sockets
    ...
    def serve(self):
        ...
        while running:
            ...
            for s in inputready:
                if s == self.server:
                    # handle the server socket correctly

                    client, address = self.server.accept()
                    print 'The chatserver got connection: connection %d is from %s' % (client.fileno(), address)
                    # Getting the  client public key and sending the server's public key
                    ...
                    #send client name back to terminal
                    self.clients += 1
                    ...
                    # Send client's joining information to other people(clients)
